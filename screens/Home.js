...
const Home = () => {
  ...
  const userInfo = () => { AsyncStorage.getItem('userInfo', (err, item) => {
      setToken(item);
    });
  }
  ...
  const onMetodeTopUpPress = () => {
    navigation.navigate('MetodeTopUp');
  };
 const onTarikSaldoPress = () => {
  navigation.navigate('TarikSaldo');
  };
  ...
  return (
    <ScrollView style={{ width: '100%' }}>
      <View>
        <Component1 namaUser={dataUser.nama}/>
        <CustomButton
          text="Langganan"
  ...
  return (
    <View>
      ...
      <View style={{ marginTop: 25, marginLeft: 27, marginRight: 25, backgroundColor: '#80D484', width: 335, height: 125, borderWidth: 1, borderColor: '#E5DEDE', borderRadius: 8 }}> 
        <Text style={{ fontSize: 20, marginTop: 15, fontWeight: 'bold', marginLeft: 30, color: '#ffffff', }}>Halo, Ganda {user} </Text>
